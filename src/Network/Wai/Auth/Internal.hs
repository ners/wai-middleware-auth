...
fixRawResponse :: OA2.OAuth2Token -> OA2.OAuth2Token
fixRawResponse token@OA2.OAuth2Token{..} =
    token{ OA2.rawResponse =  Aeson.fromList . catMaybes $
        [ Just ("access_token", Aeson.toJSON accessToken)
        , Just ("refresh_token", Aeson.toJSON refreshToken)
        , ("expires_in",) . Aeson.toJSON <$> expiresIn
        , ("token_type",) . Aeson.toJSON <$> tokenType
        , ("id_token",) . Aeson.toJSON <$> idToken
        , ("scope",) . Aeson.toJSON <$> scope
        ]
}
...
decodeToken bs =
  case decodeOrFail $ SL.fromStrict bs of
    Right (_, _, token) -> Right token
    Left (_, _, err) -> Left err

encodeToken :: OA2.OAuth2Token -> S.ByteString
encodeToken = SL.toStrict . encode
